...
var findUser = function(db, callback) {
    var cursor =db.collection('lastlab').find( );
...
var findUserwithName = function(db,callback) {
    var cursor = db.collection('lastlab').find({"fname":"Marmik"});
    cursor.each(function(err,doc) {
        assert.equal(err,null);
        if(doc != null)
        {
...
var findUserwithUniversity = function(db, callback) {
    var cursor = db.collection('lastlab').find({"education.university":"UMKC"});
    cursor.each(function(err,doc){
       assert.equal(err,null);
       if(doc != null)
       {
